   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +51929766663")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: +51929766663 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
try{
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
            ...
            console.log(color(`🌿Conectado a => ` + JSON.stringify(XeonBotInc.user, null, 2), 'yellow'))
			await delay(1999)
            console.log(chalk.yellow(`\n\n               ${chalk.bold.blue(`[ ${botname} ]`)}\n\n`))
            console.log(color(`${themeemoji} CREADOR: JOSUE\n`,'magenta'))
            await delay(1000 * 2) 
		}
...
//welcome\\ //Number +51 926 954 418 BOT WSz\\
